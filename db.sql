...
create table `product_category`(
	...
	unique key `uqe_categry_type` (`category_type`)
)comment '类目表';

create table `order_master` (
	`order_id` varchar(32) not null,
	`buyer_name` varchar not null comment '卖家名字',
	...
	`order_status` tinyint(3) not null comment default '0' '订单状态,默认0 新订单',
...
create table 'order_detail'(
	...
	`prduct_quantity` int not null comment '商品数量',
	`product_icon` varchar(128) not null,
	`create_time` timestamp not null default current_timestamp comment '创建时间',
	`update_time` timestamp not null default current_timestamp on update current_timestamp comment'更新时间',
